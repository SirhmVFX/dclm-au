...
const services = [
  { title: "Our Mission", icon: <FaHome size={24} />, description: "The DLCF has a divine mandate to win, build and commission students and staff of institutions of higher learning to be their best for the Master." },
  { title: "Bible Doctrine", icon: <FaBuilding size={24} />, description: "We earnestly contend for the faith which was once delivered unto the saints, standing firm on sound doctrine and godly living." },
  { title: "Daily Manna", icon: <FaBuilding size={24} />, description: "Access our daily devotional resource to nourish your soul, grow in the Word, and stay connected with God every day." },
];
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    ]).then(([slides, statsData, testimonialsData]) => {
        ...
        <div ref={aboutRef} className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-10">
          ...
          <div>
            ...
            <div className="p-6 md:p-8 bg-primary text-white space-y-4">
              <p>DLCF Australia is an inter-denominational fellowship, embracing campus Christians who share the same doctrinal belief irrespective of their denominations and affiliations.</p>
        ...
        <div className="w-300 mx-auto py-16 md:py-32">
          <div ref={offerRef} className="grid grid-cols-1 sm:grid-cols-2 gap-6 md:gap-10 justify-between items-end mb-8 md:mb-0">
            <div className="space-y-4 md:space-y-6">
              <p>What We Offer</p>
              <h1 className="text-3xl md:text-[50px] leading-tight md:leading-none">Your spiritual welfare is our concern.</h1>
            </div>
          ...
          <div ref={servicesRef} className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 md:gap-10 mt-8 md:mt-10">
            {services.map((service, index) => (
              <div key={index} className="p-8 md:p-10 bg-primary text-white hover:shadow-lg transition-all duration-300 hover:-translate-y-1 relative flex justify-between flex-col min-h-48">
            ...
            <div className="md:col-span-3">
              ...
              {benefits.map((benefit) => (
                <div key={benefit.id} className="border border-gray-300 p-5 md:p-6">
                  <div className="flex items-center justify-between cursor-pointer" onClick={() => toggleBenefit(benefit.id)}>
                    <div className="flex items-center gap-3">
                      <p className="text-sm text-gray-400">{benefit.id}</p>
                      <h2 className="text-lg md:text-xl font-semibold">{benefit.title}</h2>
                    </div>
                    <CgChevronDown size={20} className={`text-primary shrink-0 transition-transform duration-300 ${openBenefit === benefit.id ? "rotate-180" : ""}`} />
                  </div>
