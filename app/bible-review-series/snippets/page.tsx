...
function Snippet() {
  ...
  const bannerRef = useReveal("animate-fade-in");
  ...
  return (
    <>
      ...
      <section className="pt-32">
        <div ref={heroRef} className="w-300 mx-auto flex flex-col sm:flex-row justify-between gap-6 mb-12 md:mb-16">
          <div>
            <p>Bible Snippets</p>
      ...
      <section>
        <div className="w-300 mx-auto py-12 md:py-20">
          <div ref={gridRef} className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 md:gap-12">
            {snippets.map((snippet, index) => (
              <div key={index} className={`border border-gray-300 reveal delay-${(index % 3 + 1) * 100}`}>
      ...
      {/* ── BANNER ── */}
      <section className="w-full h-64 sm:h-96 md:h-[31rem] relative">
        <Image
          width={1600}
          height={700}
          src="/assets/12.jpg"
          ...
          className="w-full h-full object-cover"
        />
        <div ref={bannerRef} className="absolute inset-x-4 sm:inset-auto sm:left-[10%] sm:w-[420px] top-8 sm:top-12 md:top-20 bg-primary/35 text-white p-5 md:p-6">
